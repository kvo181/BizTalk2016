...
namespace BizUnit.TestSteps.BizTalk.Host
{
	...
	public class HostConductorStep : TestStepBase
	{
		...
		public override void Execute(Context context)
		{
			...
			foreach (var server in listofServers)
			{
				...
				using (mo)
				{
					...
					{
						context.LogInfo(string.Format("Starting {0} on {1}", HostInstanceName, server));
						...
						{
							context.LogInfo("BizTalk Host is already Started.");
						}
					}

					if (_action.ToLower() == "stop")
					{
						context.LogInfo(string.Format("Stopping {0} on {1}", HostInstanceName, server));
						...
						{
							context.LogInfo("BizTalk Host is already Stopped.");
						}
					}
				}
			}
		...
		private static ManagementObject GetHostInstanceWmiObject(string server, string hostName)
		{
			...
			//var scope = null == server ? new ManagementScope("root\\MicrosoftBizTalkServer", options) : new ManagementScope("\\\\" + server + "\\root\\MicrosoftBizTalkServer", options);
		    var scope = new ManagementScope("root\\MicrosoftBizTalkServer", options);
			...
			var query = new SelectQuery
							{
								QueryString =
									String.Format("SELECT * FROM MSBTS_HostInstance where HostName='" + hostName +
												  "' AND HostType=" + hostType + " AND RunningServer='" + server + "'")
							};
			...
			throw new ApplicationException(string.Format("The WMI object for the Host Instance:{0} on server:{1} could not be retrieved.", hostName, server ));
		}
	}
}
