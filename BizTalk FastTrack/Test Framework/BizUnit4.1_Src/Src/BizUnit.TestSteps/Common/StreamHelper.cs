...
namespace BizUnit.TestSteps.Common
{
	...
	public class StreamHelper
	{
		...
		/// Helper method to load a string into a MemoryStream
		/// </summary>
		/// <param name="bytes">The byte array containing the data that will be loaded into the stream</param>
		/// <returns>MemoryStream containg the data read from the byte array</returns>
		public static MemoryStream LoadMemoryStream(byte[] bytes)
		{
			var ms = new MemoryStream(bytes);
			ms.Flush();
			ms.Seek(0, SeekOrigin.Begin);
