...
namespace TestDbfLib
{
	internal class Program
	{
		...
		{
            //TestReadHttpConn();
            Console.WriteLine("Ready to create a DBF file");
			TestWriteNewDbf();
            Console.WriteLine("Ready to read the just created DBF file");
            Console.ReadKey();
            TestReadFile();
		...
		private static void TestReadFile()
		{
            ...
            // Dump the read file content to the console
            ofs = new FileStream(Path.Combine(TestPath, "filesource.txt"), FileMode.Open);
            var osr = new StreamReader(ofs, Encoding.Default);
