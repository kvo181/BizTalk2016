...
namespace bizilante.BuildGenerator.Tasks
{
    ...
    {
        /// <summary>
        /// Do not auto-increment the build number.
        /// </summary>
        NoIncrement = 0,

        /// <summary>
        /// Add one to the current build number.
        /// </summary>
        AutoIncrement = 1
    }
    ...
    /// <para/>This task is based on the AssemblyInfo task from the MSBuild Extension Pack.<para/>
    /// </summary>
    /// <remarks>
    ...
    public class AssemblyInfo : Task
    {
        ...
        /// <seealso cref="AssemblyFileBuildNumberFormat"/>
        /// <seealso cref="IncrementMethod"/>
        public string AssemblyFileMinorVersionType { get; set; }
        ...
        public override bool Execute()
        {
            ...
            foreach (ITaskItem item in this.AssemblyInfoFiles)
            {
                ...
                if (!this.SkipVersioning)
                {
                    ...
                    try
                    {
                        this.UpdateAssemblyVersion(versionToUpdate, this.assemblyVersionSettings, "AssemblyVersion", false);
                    }
                    catch (Exception ex)
                    {
                        Log.LogError(ex.Message, new object[] { });
                        return false;
                    }
                    ...
                    try
                    {
                        this.UpdateAssemblyVersion(versionToUpdate, this.assemblyFileVersionSettings, "AssemblyFileVersion", true);
                    }
                    catch (Exception ex)
                    {
                        Log.LogError(ex.Message, new object[] { });
        ...
        private void UpdateAssemblyVersion(Version versionToUpdate, AssemblyVersionSettings requestedVersion, string dialogTitle, bool showDialog)
        {
            ...
            if (requestedVersion.MajorVersion != null)
            {
                ...
                if (int.Parse(requestedVersion.MajorVersion) > int.Parse(versionToUpdate.MajorVersion))
                {
                    // When the major version number flips a number, we reset the others to 0.
                    versionToUpdate.MinorVersion = "0";
                    versionToUpdate.BuildNumber = "0";
            ...
            if (requestedVersion.MinorVersion != null)
            {
                ...
                if (int.Parse(requestedVersion.MinorVersion) > int.Parse(versionToUpdate.MinorVersion))
                {
                    // When the minor version number flips a number, we reset the others to 0.
                    versionToUpdate.BuildNumber = "0";
                    versionToUpdate.Revision = "0";
                }
            ...
            // We start with the buildnumber and version found in the assemblyinfo file
            requestedVersion.BuildNumber = versionToUpdate.BuildNumber;
            requestedVersion.Revision = versionToUpdate.Revision;

            if (showDialog)
            {
                // Display a window allowing the build user to specify the build number and revision.
                VersionForm versionForm = new VersionForm(dialogTitle, versionToUpdate.MajorVersion, versionToUpdate.MinorVersion, versionToUpdate.BuildNumber, versionToUpdate.Revision);
                versionForm.MinorIncrement = (requestedVersion.MinorVersionType == IncrementMethod.AutoIncrement);
                versionForm.BuildIncrement = (requestedVersion.BuildNumberType == IncrementMethod.AutoIncrement);
                versionForm.RevisionIncrement = (requestedVersion.RevisionType == IncrementMethod.AutoIncrement);
                if (versionForm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    requestedVersion.MinorVersion = versionForm.MinorVersion.ToString();
                    requestedVersion.BuildNumber = versionForm.BuildNumber.ToString();
                    requestedVersion.Revision = versionForm.Revision.ToString();
                }
                else
                    throw new VersionException(string.Format("{0}: Version update cancelled", dialogTitle)); 
            }
            ...
            bool handleSpecialInteraction = (requestedVersion.RevisionType == IncrementMethod.AutoIncrement);
            if (handleSpecialInteraction)
            {
                originalBuildNumber = versionToUpdate.BuildNumber;
            }

            // Go ahead and update the Minor version.
            versionToUpdate.MinorVersion = this.UpdateVersionProperty(versionToUpdate.MinorVersion, requestedVersion.MinorVersionType, requestedVersion.MinorVersion, requestedVersion.MinorVersionFormat, "\tUpdating minor version to {0}");
        ...
        private bool ParseIncrementProperties()
        {
            ...
            // Handle AssemblyFileMinorVersionType
            if (this.AssemblyFileMinorVersionType == null)
            {
                this.assemblyFileVersionSettings.MinorVersionType = IncrementMethod.NoIncrement;
            }
            else
            {
                if (!Enum.IsDefined(typeof(IncrementMethod), this.AssemblyFileMinorVersionType))
                {
                    this.Log.LogError("The value specified for AssemblyFileMinorVersionType is invalid. It must be one of: {0}", enumNames);

                    return false;
                }

                this.assemblyFileVersionSettings.MinorVersionType = (IncrementMethod)Enum.Parse(typeof(IncrementMethod), this.AssemblyFileBuildNumberType);
            }
        ...
        private struct AssemblyVersionSettings
        {
            ...
            public string MinorVersionFormat;
            public IncrementMethod MinorVersionType;
